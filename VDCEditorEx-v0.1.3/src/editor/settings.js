...
function SetupTemplatesPanelEvents() {
    ...
    OkayBtn?.addEventListener('click', (e) => {
        ...
        const Content = document.getElementById("VDCEditorEx-NoDataDlg-Content");
    ...
    ExportUDTBtn.addEventListener("click", (e) => {
        chrome.storage.local.get("VDC-UDT", (result) => {
            ...
            if (!data || jsonString == "{}") {
                const BlurDlg = document.getElementById("VDCEditorEx-BlurDlg");
                const Content = document.getElementById("VDCEditorEx-NoDataDlg-Content");
                const TextDlg = document.getElementById("Dialog-Info-Text");
                BlurDlg.style.display = "block";
                Content.style.display = "block";
                TextDlg.textContent = "No data was found while trying to export."
                return;
            }
            ...
            try {
                if (file.name.endsWith(".json")) {
                    ...
                    if (blockKeys.length == 0) {
                        BlurDlg.style.display = "block";
                        Content.style.display = "block";
                        TextDlg.textContent = "Invalid JSON structure, does not match old version or the new version.";
                        return;
                    }
                    ...
                    BlurDlg.style.display = "block";
                    Content.style.display = "block";
                    TextDlg.textContent = "Unsupported file, please use a JSON file."
                    return;
                }
                ...
                BlurDlg.style.display = "block";
                Content.style.display = "block";
                TextDlg.textContent = err?.message || "Failed to parse the file, please check for any errors is the file.";
            }
        };
...
function CreateNewTemplateBlock(templateCount, UDTNameLen = 1, UDTParamLen = 1) {
    ...
    NameInput.id = "Block-" + templateCount + "-Index-" + "1"; // Index means how many inputs are there, there always must be 1, without it, the user name won't exist
    NameInput.type = "text";
    ...
    ParamInput.id = "Block-" + templateCount + "-Params-Index-" + "1"; // Index means how many inputs are there, there always must be 1, without it, the user name won't exist
    ...
    ParamColor.id = "Block-" + templateCount + "-Params-Clr-Index-" + "1"; // Index means how many inputs are there, there always must be 1, without it, the user name won't exist
