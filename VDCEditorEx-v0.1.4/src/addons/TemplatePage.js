...
const url = "https://developer.valvesoftware.com/wiki/"
...
function TemplatePage_addEventListener() {
    ...
    templatePageHandler = function (e) {
        ...
        if (templateName && !templateName.includes("{")
            ...
            let newUrl = `https://developer.valvesoftware.com/wiki/Template:${wikiEncode(capitalizeFirst(templateName))}`;;

            const nonTemplateNamespaces = /^(File|Category|Image|Media|Help|Portal|Special|User):/i;
            let cleanName = templateName.replace(/^:/, "");

            if (nonTemplateNamespaces.test(cleanName) || templateName.startsWith(":")) {
                newUrl = url + wikiEncode(capitalizeFirst(cleanName));
            }
