...
const EditorSettings = new Proxy({
	...
	BracketColorDepth: false,
    HTMLUnsupportedTags: false,
	HTMLInvalidTags: false,
	ShowAllChars: false,
	MwTables: false,
	ShowLiveSummary: false,
	EDITOR_DIR_RTL: false, // Buggy, breaks the Show All Chars feature.
...
function DuplicateEditorWarn() {
	...
	message.className = "mw-message-box";
	message.style = "background-color: #442222; border-color: #b42e2e;";
...
function PageTooLongWarn() {
	let content = document.getElementById("bodyContent");

	let message = document.createElement("div");
	message.className = "mw-message-box";
	message.style = "background-color: #442222; border-color: #b42e2e;";
	message.innerHTML = `<p><strong>Editor disabled:</strong> page content exceeds the maximum supported length. (100,000 characters)</p>`;
...
function Func_UpdateSizes() {
	...
	[Div_Editor, Div_StylizedCode, Div_SameSelection, Div_CodeLines, Div_SearchCode].forEach(el => {
			el.style.paddingRight = EditorSettings.EDITOR_DIR_RTL ? pad : padding;
			el.style.paddingLeft = EditorSettings.EDITOR_DIR_RTL ? padding : pad;
	});
...
function simpleHash64(str) {
    ...
    return (h & BigInt("0xffffffffffffffff")).toString(16);
}
...
function maskComments(text) {
    ...
    const masked = text.replace(/&lt;!--[\s\S]*?--&gt;/g, (match) => {
        const hash = simpleHash64(match);
        comments.set(hash, match);
        return `__COMMENT_${hash}__`;
    });
...
function restoreComments(text, comments) {
    return text.replace(/__COMMENT_([a-f0-9]+)__/g, (_, hash) => {
        return comments.get(hash) ?? _;
    });
}
	...
	try {
		if (EditorSettings.SameSelection && window.getSelection().focusNode.parentNode.id == "VDCEditorEx-Editor") {
			if (window.getSelection().toString().length !== 0) {
				...
				if (['{', '}', '[', ']', '(', ')'].includes(selectedText) && EditorSettings.MatchingBracket) {
	...
	switch (e.keyCode) {
			...
			if (e.ctrlKey) {
				...
				if (BROWSER_TYPE != "Firefox" && sel.toString().length == 0) {
					...
					for (var i = 0; i < Lines.length; i++) {
						...
						if (TextAmount >= pos.pos) { return true; }
		...
		case 68: // D
			if (e.ctrlKey) {
				...
				let pos = getCursorPosition(Div_Editor, node, offset, { pos: 0, done: false });
				...
				for (let i = 0; i < Lines.length; i++) {
					...
					if (TextAmount >= pos.pos) {
						if (sel == 0) {
							let CurrentLine = Div_Editor.textContent.substring(TextAmount - Lines[i].length, TextAmount);
		...
		case 9: // Tab
		{
			...
			let anchor = getCursorPosition(Div_Editor, sel.anchorNode, sel.anchorOffset, { pos: 0, done: false });
			let focus  = getCursorPosition(Div_Editor, sel.focusNode, sel.focusOffset, { pos: 0, done: false });

			let start = Math.min(anchor.pos, focus.pos);
			let end   = Math.max(anchor.pos, focus.pos);

			let text = Div_Editor.textContent;

			let lineStart = text.lastIndexOf('\n', start - 1) + 1;
			let lineEndIndex = text.indexOf('\n', end);
			let lineEnd = lineEndIndex === -1 ? text.length : lineEndIndex;
			...
			let startRange = setCursorPosition(Div_Editor, range, { pos: lineStart, done: false });

			let endRange = setCursorPosition(Div_Editor, document.createRange(), { pos: lineEnd, done: false });
			startRange.setEnd(endRange.endContainer, endRange.endOffset);

			sel.removeAllRanges();
			sel.addRange(startRange);

			document.execCommand('insertHTML', false, encodeHTML(newBlock));

			range = document.createRange();
			startRange = setCursorPosition(Div_Editor, range, { pos: lineStart, done: false });
			
			let newBlockLength = newBlock.length;
			endRange = setCursorPosition(Div_Editor, document.createRange(), { pos: lineStart + newBlockLength, done: false });
			startRange.setEnd(endRange.endContainer, endRange.endOffset);
		...
		case 13: // Enter
		{
			...
			for (let i = 0; i < Lines.length; i++) {
				...
				if (TextAmount >= pos.pos) {
					TextAmount -= Lines[i].length;
					let CurrentLine = Div_Editor.textContent.substring(TextAmount, pos.pos)
...
async function AsyncSummaryColorLinks(text) {
	...
	async function isValid(value, linkName, linkShow) {
		...
		if (value == true) {
			...
			let InterWikiLink = renderInterwiki(linkName, linkShow);
