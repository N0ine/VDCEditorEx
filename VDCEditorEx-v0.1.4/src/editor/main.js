...
if (document.body.querySelector('form textarea#wpTextbox1')) {
    const IntervalId = setInterval(() => {
        if (document.body.querySelector(".wikiEditor-ui")) {
            ...
            if (document.getElementById('wpTextbox1') && document.getElementById('wpTextbox1').value.length >= 100_000) {
                PageTooLongWarn();
                return;
            }
...
async function InitTextEditorLogic() {
    ...
    document.getElementById("VDC-T-ReplaceLinksWTemp").addEventListener('click', (e) => {
        e.preventDefault();
        ConfirmDialog("Are you sure you want to replace links with templates?", () => {
    ...
    document.getElementById("VDC-T-ReplaceTempWLinks").addEventListener('click', (e) => {
        e.preventDefault();
        ConfirmDialog("Are you sure you want to replace templates with links?", () => {
            let TextEditor = Div_Editor.innerHTML;
            
            TextEditor = TextEditor.replace(/{{[Ll]\|(.+?)}}/g, "[[$1]]")
            TextEditor = TextEditor.replace(/{{[Ll]Category\|(.+?)}}/g, "[[:Category:$1]]")
            TextEditor = TextEditor.replace(/{{[Aa]Category\|(.+?)}}/g, "[[Category:$1]]")
            TextEditor = TextEditor.replace(/{{[Ll]Help\|(.+?)}}/g, "[[Help:$1]]")
            TextEditor = TextEditor.replace(/{{[Ll]Project\|(.+?)}}/g, "[[Project:$1]]")
            TextEditor = TextEditor.replace(/{{[Ll]Special\|(.+?)}}/g, "[[Special:$1]]")
