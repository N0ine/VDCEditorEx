...
function EditorMain()
{
	...
	function EditorFormatting()
	{
		...
		Span_CodeLength.innerHTML = getTranslation("Length").replace("$1", Text.length - 1);
	...
	function Func_SelectionChange()
	{
		try
		{
			...
			SelectionCount.innerHTML = getTranslation("Found").replace("$1", SameSelectionCount.length);
			...
			setTimeout(() => { 
				...
				if (Div_Editor.innerHTML.charAt(Div_Editor.innerHTML.length - 1) != '\n')
					Span_CaretPosition.textContent = getTranslation("PosLength").replace("$1", Math.min(pos.pos, Div_Editor.textContent.length));
				else
					Span_CaretPosition.textContent = getTranslation("PosLength").replace("$1", Math.min(pos.pos, Div_Editor.textContent.length - 1));
			}, 0);
		}
		catch (e) {}
	}
	...
	listener.onkeydown = function (ev) {
		...
		Span_CaretPosition.textContent = getTranslation("PosLength").replace("$1", caretPos);
